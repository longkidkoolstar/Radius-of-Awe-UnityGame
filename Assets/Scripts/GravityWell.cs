...
/// <summary>
/// A puzzle gravity trap/elevator element. Place a trigger BoxCollider2D in the scene.
/// In the mundane world, it is inert. In the Wonder world, it:
///   1. Spawns falling glowing flow particles in memory.
///   2. Applies a steady downward force to the Player or Floaty Crates!
/// </summary>
[RequireComponent(typeof(BoxCollider2D))]
public class GravityWell : MonoBehaviour
{
    [Header("Gravity Well Physics")]
    [Tooltip("Amount of downward force to apply to Rigidbody2D objects inside the tunnel.")]
    [SerializeField] private float downwardForce = 80f;
    [Tooltip("Maximum downward velocity a Rigidbody2D can reach while being pulled (keeps it stable).")]
    [SerializeField] private float maxDownVelocity = 7.5f;
    ...
    [Tooltip("Vibrant color of the downward glowing gravity streams.")]
    [SerializeField] private Color flowColor = new Color(1.0f, 0.25f, 0.05f, 0.45f); // Neon Red-Orange
    [Tooltip("Falling speed of the flow visual lines.")]
    [SerializeField] private float flowSpeed = 3.2f;
    ...
    private void SetupFlowLines()
    {
        ...
        {
            GameObject go = new GameObject("GravityWellFlowLine");
    ...
    private void Update()
    {
        // Query the static API using the gravity well's trigger collider bounds
        ...
        foreach (var line in flowLines)
        {
            ...
            if (isActiveInWonder)
            {
                ...
                // Recycle back to top when leaving
    ...
    private void FixedUpdate()
    {
        ...
        // Apply physical downward forces to Rigidbody2Ds
        for (int i = rigidbodiesInRange.Count - 1; i >= 0; i--)
        {
            ...
            // Apply direct downward force up to a terminal speed
